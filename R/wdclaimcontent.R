...
wdclaimcontent.int <- function(url, lang) {
	...
	for(cl in 1:n.claims) {
		...
		if(n.claims>1) {
			...
			wdcontent$claims[[cl]] <- list(guid=guid, type=type, content=content)
		} else wdcontent <- list(guid=guid, item=item, property=prop, type=type, content=content)
	}
